...
def completeArray(arr, value=None, desired_length=2):
...
def extractZCandCity(raw : str):
  raw = re.sub(r'\n|\r|\xa0', '', raw)
  zip_code = re.search(r'\d{5}', raw)
  if zip_code:
    zip_code = zip_code.group(0)
    city = re.search(r'\w[^0-9]', raw)
    if city:
      city = city.group(0)
  return {"cp": zip_code, "city":city}
